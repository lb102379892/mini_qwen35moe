...
#include "ggml-cuda/ssm-conv-step.cuh"
#include "ggml-cuda/deltanet-decode-proj.cuh"
...
static bool ggml_cuda_compute_forward(ggml_backend_cuda_context & ctx, struct ggml_tensor * dst) {
    switch (dst->op) {
        ...
        case GGML_OP_GATED_DELTA_NET:
            ...
            break;
        case GGML_OP_DELTANET_DECODE_PROJ:
            ggml_cuda_op_deltanet_decode_proj(ctx, dst);
            break;
...
static bool ggml_cuda_can_fuse(const struct ggml_cgraph *                cgraph,
    ...
    if (ops.size() == 2 && ops.begin()[0] == GGML_OP_SSM_CONV_STEP && ops.begin()[1] == GGML_OP_UNARY
     && unary_ops.size() == 1 && unary_ops.begin()[0] == GGML_UNARY_OP_SILU) {
        const ggml_tensor * ssm_step = cgraph->nodes[node_idx];
        const ggml_tensor * silu     = cgraph->nodes[node_idx + 1];
        if (ggml_get_unary_op(silu) != unary_ops.begin()[0]) {
            return false;
        }

        if (ssm_step->type != GGML_TYPE_F32 || silu->type != GGML_TYPE_F32) {
            return false;
        }
...
static int ggml_cuda_try_fuse(ggml_backend_cuda_context * cuda_ctx, ggml_cgraph * cgraph, int i) {
    ...
    if (ggml_cuda_can_fuse(cgraph, i, { GGML_OP_SSM_CONV_STEP, GGML_OP_UNARY }, { GGML_UNARY_OP_SILU })) {
        ggml_cuda_op_ssm_conv_step(*cuda_ctx, node, cgraph->nodes[i + 1]);
        return 1;
    }
...
static bool ggml_backend_cuda_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_DELTANET_DECODE_PROJ: {
            ...
            const int cc = ggml_cuda_info().devices[dev_ctx->device].cc;
            for (int i = 1; i <= 4; ++i) {
                if (!ggml_is_quantized(op->src[i]->type)) {
                    return false;
                }
                if (!ggml_cuda_should_use_mmvq(op->src[i]->type, cc, 1)) {
                    return false;
                }
            }
            return true;
